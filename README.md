...
## Render import/start command notes

This project is a flat FastAPI app: `app.py` and `main.py` live at the repository root. Use one of these start commands:
...
Do not use `uvicorn app.main:app`; there is no `app/` package directory in this project.

The compatibility endpoint `POST /api/run` is registered by `main.py`, so deploy with `main:app` if older clients call `/api/run`.
