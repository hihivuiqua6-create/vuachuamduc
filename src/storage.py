...
_DEFAULT = {
    ...
    "cookies": [],            # [{"id","label","cookie_string","user_agent","created_at","active","last_used"}]
...
def _load() -> dict[str, Any]:
    ...
    try:
        with open(DB_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        # migrate old shape ({"user": {...}}) → {"users":[...]}
        if "users" not in data:
            old_u = data.pop("user", None)
            data["users"] = []
            if old_u:
                old_u.setdefault("role", "admin")
                data["users"].append(old_u)
