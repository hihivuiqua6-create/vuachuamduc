...
class ZefoySessionError(Exception):
    """Raised when the Zefoy session isn't usable (needs captcha / blocked / expired)."""
...
def _diagnose_home(html_content: str) -> ZefoySessionError | None:
    low = (html_content or "").lower()
    if not low.strip():
        return ZefoySessionError("empty", "Zefoy trả về rỗng — cookie/UA có thể sai hoặc bị chặn IP.", 502)
    if "just a moment" in low or "cf-chl" in low or "challenge-platform" in low:
        return ZefoySessionError(
            "cloudflare",
            "Đang bị Cloudflare chặn. Cookie phải chứa cf_clearance hợp lệ (và User-Agent phải khớp trình duyệt lấy cookie).",
            502,
        )
    if "captcha-img" in low or "_captcha=" in low or 'name="captchalogin"' in low:
        return ZefoySessionError(
            "captcha_required",
            "Cookie chưa qua bước captcha. Mở zefoy.com trên trình duyệt, giải captcha đến khi thấy danh sách dịch vụ rồi copy lại toàn bộ cookie (gồm PHPSESSID + cf_clearance sau captcha).",
            400,
        )
    return None
...
def get_services(session: requests.Session) -> tuple[list[dict], str]:
    ...
    if not services:
        err = _diagnose_home(html_content)
        if err:
            raise err
        raise ZefoySessionError(
            "no_services",
            "Không thấy dịch vụ trên trang chủ Zefoy (có thể Zefoy đổi giao diện hoặc cookie đã hết hạn).",
            502,
        )
