...
from .zefoy_core import build_session, get_services, run_boost, ZefoySessionError
...
def api_services(claims: dict = Depends(require_user)):
    ...
    except ZefoySessionError as e:
        raise HTTPException(e.status_code, f"[{e.code}] {e.message}")
    return {
